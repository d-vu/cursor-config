...
## 1. Authenticate (same as machine A)
...
## → GitHub.com → HTTPS → login with browser, as d-vu
...
```
alias cpush='cd ~/cursor-config && cursor --list-extensions > extensions.txt && git add -A && git commit -m "sync $(date +%m/%d-%H:%M)" && git push && cd -'
